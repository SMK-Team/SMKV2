...
BossRaehan.ev.on('group-participants.update', async (anu) => {
	...
	let num = anu.participants[0]
	 let mdata = await BossRaehan.groupMetadata(anu.jid)
     let memeg = mdata.participants.length
      let v = BossRaehan.contacts[num] || { notify: num.replace(/@.+/, "") }
      let anu_user = v.vname || v.notify || num.split("@")[0]
     let time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
	  try {
	  ppimg = await BossRaehan.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
	  } catch {
...
𝘿𝙄 𝙂𝙍𝙐𝙋 𝙄𝙉𝙄`
	  ...
	  BossRaehan.sendMessage(mdata.id, image, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
      } else if (anu.action == 'remove') {
	  let num = anu.participants[0]
	 let mdata = await BossRaehan.groupMetadata(anu.jid)
      let memeg = mdata.participants.length
      let w = BossRaehan.contacts[num] || { notify: num.replace(/@.+/, "") }
      let anu_user = w.vname || w.notify || num.split("@")[0]
    let time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
	  try {
	  ppimg = await BossRaehan.getProfilePicture(`${num.split('@')[0]}@c.us`)
	  } catch {
	  ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
	  }
	  image = await getBuffer(
	  ...
	  let teks = `𝙎𝙀𝙇𝘼𝙈𝘼𝙏 𝙏𝙄𝙉𝙂𝙂𝘼𝙇 @${num.split('@')[0]}\n𝘽𝙏𝙒 𝙅𝘼𝙉𝙂𝘼𝙉 𝘽𝘼𝙇𝙄𝙆 𝙇𝘼𝙂𝙄 𝙔𝘼𝙆`
	  let buff = await getBuffer(ppimg)
	  BossRaehan.sendMessage(mdata.id, image, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
      }
            }
